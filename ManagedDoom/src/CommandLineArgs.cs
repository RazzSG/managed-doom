...
namespace ManagedDoom
{
    public sealed class CommandLineArgs
    {
        ...
        public readonly Arg<GameCompatibilityMode> compatibility;
        ...
        public CommandLineArgs(string[] args)
        {
            ...
            compatibility = Check_compatibility(args);
        ...
        private static Arg<GameCompatibilityMode> Check_compatibility(string[] args)
        {
            var value = GetString(args, "-compatibility");
            if (!value.Present)
                return new Arg<GameCompatibilityMode>();

            if (Enum.TryParse<GameCompatibilityMode>(value.Value, true, out var mode) &&
                Enum.IsDefined(mode))
            {
                return new Arg<GameCompatibilityMode>(mode);
            }

            throw new ArgumentException($"Unknown compatibility mode: {value.Value}. Expected auto, vanilla, boom, mbf, or mbf21.");
        }
