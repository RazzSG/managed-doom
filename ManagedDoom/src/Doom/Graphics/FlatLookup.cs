...
namespace ManagedDoom
{
    public sealed class FlatLookup : IFlatLookup
    {
        ...
        {
            var fStartName = DoomString.Resolve("F_START");
            var fEndName = DoomString.Resolve("F_END");

            var fStartCount = CountLump(wad, fStartName);
            var fEndCount = CountLump(wad, fEndName);
            ...
            {
                InitStandard(wad, fStartName, fEndName);
            }
            else if (deutexMerge)
            {
                InitDeuTexMerge(wad, fStartName, fEndName);
            }
            else
            {
        ...
        private void InitStandard(Wad wad, string fStartName, string fEndName)
        {
            try
            {
                Console.Write("Load flats: ");

                var firstFlat = wad.GetLumpNumber(fStartName) + 1;
                var lastFlat = wad.GetLumpNumber(fEndName) - 1;
                ...
                var skyFlatName = DoomString.Resolve("F_SKY1");
                skyFlatNumber = nameToNumber[skyFlatName];
                skyFlat = nameToFlat[skyFlatName];
        ...
        private void InitDeuTexMerge(Wad wad, string fStartName, string fEndName)
        {
            try
            {
                Console.Write("Load flats: ");
                ...
                for (var lump = 0; lump < wad.LumpInfos.Count; lump++)
                {
                    ...
                    {
                        if (name == fStartName || name == "FF_START")
                        {
                            flatZone = true;
                        }
                    }
