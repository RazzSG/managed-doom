...
namespace ManagedDoom
{
    public sealed class Patch
    {
        ...
        {
            // Classic DeHackEd Text may rename graphic lump names. Resolve
            // only when the patch is loaded; render-time users keep their
            // existing cached Patch instances.
            var resolved = DoomString.Resolve(name);
            return FromData(resolved, wad.ReadLump(resolved));
        }
