...
namespace ManagedDoom
{
    public sealed class OpeningSequence
    {
        ...
        public UpdateResult Update()
        {
            ...
            switch (currentStage)
            {
                ...
                case 5:
                    if (!demo.ReadCmd(cmds))
                    {
                        if (content.Wad.GetLumpNumber(ResolveDemoLumpName("demo4")) == -1)
                        {
                            nextStage = 0;
                        }
                        else
        ...
        private void StartDemo(string lump)
        {
            ...
            var resolvedLump = ResolveDemoLumpName(lump);
            demo = new Demo(content.Wad.ReadLump(resolvedLump));
        ...
        private static string ResolveDemoLumpName(string dehName)
        {
            return DoomString.Resolve(dehName).ToUpperInvariant();
        }
