...
namespace ManagedDoom
{
    public sealed class LightingChange
    {
        ...
        public void SpawnFireFlicker(Sector sector)
        {
            ...
            ClearSectorLightingSpecial(sector);
        ...
        private void ClearSectorLightingSpecial(Sector sector)
        {
            if (GameCompatibilityFeatures.SupportsGeneralizedSectorSpecials(world.Options.Compatibility))
            {
                sector.Special = BoomSectorSpecialDecoder.ClearLightingSpecial(sector.Special);
            }
            else
            {
                sector.Special = 0;
