...
namespace ManagedDoom
{
	public sealed class MapInteraction
	{
		...
		private bool UseTraverse(Intercept intercept)
		{
			...
			// Boom PassThru lets the same use trace continue on to additional
			// special lines within range. Vanilla always stops at the first one.
			return BoomUseTraversal.ShouldContinueAfterActivation(
				intercept.Line,
		...
		public bool UseSpecialLine(Mobj thing, LineDef line, int side)
		{
			if (GameCompatibilityFeatures.SupportsBoomLineSpecials(world.Options.Compatibility) &&
				BoomLineSpecials.TryUse(world, line, side, thing, out var boomResult))
			{
				return boomResult;
			}
		...
		{
			// Things that should never activate crossing specials.
			if (thing.Player == null)
			{
				switch (thing.Type)
				{
		...
		{
			if (GameCompatibilityFeatures.SupportsBoomLineSpecials(world.Options.Compatibility) &&
				BoomLineSpecials.TryShoot(world, line, thing))
			{
				return;
			}
