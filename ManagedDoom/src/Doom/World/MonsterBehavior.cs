...
namespace ManagedDoom
{
    public sealed class MonsterBehavior
    {
        ...
        private bool Move(Mobj actor)
        {
            ...
            if (!tryOk)
            {
                ...
                while (tm.crossedSpecialCount-- > 0)
                {
                    ...
                    // it does not contribute to the successful attempt.
                    if (world.MapInteraction.UseSpecialLine(actor, line, 0))
                    {
                        activatedFlags |= ReferenceEquals(line, tm.BlockingLine)
                            ? BoomDoorCompatibility.BlockingLineActivated
                            : BoomDoorCompatibility.OtherLineActivated;
                    }
                }
                ...
                return BoomDoorCompatibility.ShouldContinueAfterBlockedDoorAttempt(
                    compatibility,
                    activatedFlags,
                    world.Random.Next());
            }
        ...
        public void SpawnFly(Mobj actor)
        {
            ...
            world.ThingMovement.TeleportMove(monster, monster.X, monster.Y, bossTeleport: true);
