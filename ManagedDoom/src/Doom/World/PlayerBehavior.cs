    ...
    public sealed class PlayerBehavior
    {
        ...
        private void PlayerInSpecialSector(Player player)
        {
            ...
            if (GameCompatibilityFeatures.SupportsGeneralizedSectorSpecials(world.Options.Compatibility) &&
                rawSpecial >= 32)
            {
                var special = BoomSectorSpecialDecoder.Decode(rawSpecial);
                BoomSectorDamageHandler.Apply(world, player, special);
                BoomSectorSecretHandler.Reveal(player, sector, special);
                return;
            }
            ...
            switch (rawSpecial)
            {
                ...
                default:
                    throw new Exception("Unknown sector special: " + rawSpecial);
            }
        }
