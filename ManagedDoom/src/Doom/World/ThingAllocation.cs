...
namespace ManagedDoom
{
    public sealed class ThingAllocation
    {
        ...
        public void SpawnMapThing(MapThing mt)
        {
            ...
            // Boom point push/pull markers are invisible control objects. The compatibility
            // layer resolves them from map things directly, so they must not fall through
            // the vanilla DoomInfo lookup where 5001/5002 are unknown.
            if (GameCompatibilityFeatures.SupportsBoom(world.Options.Compatibility) &&
                (mt.Type == BoomPointPusher.PushSourceThingType ||
                 mt.Type == BoomPointPusher.PullSourceThingType))
            {
                return;
            }
        ...
        public void RemoveMobj(Mobj mobj)
        {
            ...
            tm.RemoveTouchingSectorLinks(mobj);
