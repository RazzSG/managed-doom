...
namespace ManagedDoom
{
	public sealed class VisibilityCheck
	{
		...
		{
			sightZStart = looker.Z + looker.Height - (looker.Height >> 2);
			topSlope = pointZ + pointHeight - sightZStart;
			bottomSlope = pointZ - sightZStart;

			trace.X = looker.X;
			trace.Y = looker.Y;
			trace.Dx = pointX - looker.X;
			trace.Dy = pointY - looker.Y;
		...
		/// <summary>
		/// Returns true if a straight line from the looker to a static target point is unobstructed.
		/// </summary>
		public bool CheckSightToPoint(
			Mobj looker,
			Fixed pointX,
			Fixed pointY,
			Fixed pointZ,
			...
			Sector pointSector)
		{
			...
			if (map.Reject.Check(looker.Subsector.Sector, pointSector))
				return false;

			SetupTrace(looker, pointX, pointY, pointZ, pointHeight);
			return CrossBlockMap();
		}
	}
}
