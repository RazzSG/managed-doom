...
namespace ManagedDoom.Video
{
    public sealed class StatusBarRenderer
    {
        ...
        public void Render(Player player, bool drawBackground)
        {
            ...
            var compatibility = player.Mobj.World.Options.Compatibility;
            for (var i = 0; i < BoomStatusBarKeys.ColorCount; i++)
            {
                var patchIndex = BoomStatusBarKeys.ResolvePatchIndex(
                    i,
                    player.Cards[i],
                    player.Cards[i + BoomStatusBarKeys.ColorCount],
                    compatibility);

                if (patchIndex != -1)
                    DrawMultIcon(keys[i], patchIndex);
            }
        }
        ...
        private class Patches
        {
            ...
            public Patches(Wad wad)
            {
                ...
                Keys = new Patch[BoomStatusBarKeys.PatchCount];
                for (var i = 0; i < BoomStatusBarKeys.ClassicPatchCount; i++)
                {
                    Keys[i] = Patch.FromWad(wad, "STKEYS" + i);
                }

                for (var i = BoomStatusBarKeys.CombinedPatchBase; i < Keys.Length; i++)
                {
                    var name = "STKEYS" + i;
                    var resolvedName = DoomString.Resolve(name);

                    // Boom treats STKEYS6..8 as predefined resources, but a
                    // loaded WAD is allowed to replace them. Prefer the WAD
                    // version when present; otherwise use our embedded copy.
                    Keys[i] = wad.GetLumpNumber(resolvedName) != -1
                        ? Patch.FromWad(wad, name)
                        : BoomStatusBarKeyResources.LoadPatch(i);
                }
